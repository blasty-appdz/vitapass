...
// ─── HELPER : génère le QR canvas depuis qrcodejs ────────────────────────────
...
function QRScreen({ nav, profile, dossierData }) {
  ...
  const showLocalToast = (msg) => { setToastMsg(msg); setTimeout(() => setToastMsg(null), 3000) }
  ...
  // ── Fond d'écran 1080×1920 ──────────────────────────────────────────────────
  const downloadWallpaper = async () => {
    setGenWallpaper(true)
    try {
      // Attendre que qrcodejs ait fini de générer l'image
      await new Promise(r => setTimeout(r, 300))
      const qrDataUrl = await getQRDataUrl(qrRef)
      if (!qrDataUrl) { showLocalToast('QR non disponible — activez le QR Pass d\'abord'); setGenWallpaper(false); return }
  ...
  // ── Carte urgence format bancaire ───────────────────────────────────────────
  const downloadCard = async () => {
    setGenCard(true)
    try {
      await new Promise(r => setTimeout(r, 300))
      const qrDataUrl = await getQRDataUrl(qrRef)
      if (!qrDataUrl) { showLocalToast('QR non disponible — activez le QR Pass d\'abord'); setGenCard(false); return }

      const canvas = document.createElement('canvas')
      // 85.6mm x 54mm à 300dpi
      canvas.width = 1011
      canvas.height = 638
      const ctx = canvas.getContext('2d')
      ...
      if (dossierData?.allergies) {
        const a = typeof dossierData.allergies === 'string' ? dossierData.allergies : JSON.stringify(dossierData.allergies)
        if (a && a !== '[]' && a !== 'null') {
          ...
          <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,padding:16,display:'flex',flexDirection:'column',gap:10}}>
            ...
            <div style={{background:'var(--card2)',borderRadius:12,padding:14}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                ...
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:13,fontWeight:700,color:'var(--white)'}}>Fond d'écran verrouillage</div>
                  <div style={{fontSize:11,color:'var(--dim)',marginTop:2}}>Visible sans déverrouiller votre téléphone</div>
                </div>
              </div>
            ...
            <div style={{background:'var(--card2)',borderRadius:12,padding:14}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                <span style={{fontSize:22}}>🪪</span>
                <div>
                  <div style={{fontFamily:"'Syne',sans-serif",fontSize:13,fontWeight:700,color:'var(--white)'}}>Carte d'urgence</div>
                  <div style={{fontSize:11,color:'var(--dim)',marginTop:2}}>Format carte bancaire — à glisser dans le portefeuille</div>
                </div>
              </div>
              <div style={{display:'flex',gap:6,marginBottom:10,flexWrap:'wrap'}}>
...
function UrgenceToggle({ dossier, userId, onToggle }) {
  ...
  const toggle = async () => {
    ...
    // Si on active et pas encore de token, en créer un
