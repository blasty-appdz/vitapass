...
function getAvatarEmoji(gender, role) {
  if (role === "doctor") return gender === "Féminin" ? "👩‍⚕️" : "👨‍";
...
export default function DoctorDashboard({ nav, showToast }) {
  ...
  async function fetchData() {
    ...
    const { data: accesses, error } = await supabase.from("doctor_access").select("id, access_level, granted_at, patient_id").eq("doctor_id", user.id);
    if (error || !accesses || accesses.length === 0) { setPatients([]); setLoading(false); return; }

    const patientIds = accesses.map(a => a.patient_id);
    const { data: patientProfiles } = await supabase.from("profiles").select("id, fname, lname, dob, blood, gender").in("id", patientIds);
  ...
  function getFullName(p) { if (!p) return "–"; return `${p.fname || ""} ${p.lname || ""}`.trim() || "–"; }
  function getAge(dob) { if (!dob) return "–"; return Math.floor((Date.now() - new Date(dob)) / (1000 * 60 * 60 * 24 * 365.25)) + " ans"; }
  function formatDate(ts) { if (!ts) return "–"; return new Date(ts).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" }); }
      ...
      <div style={styles.sidebarBottom}>
        <div style={styles.doctorCard}>
          <div style={styles.doctorAvatar}>
            {getAvatarEmoji(doctor?.gender, "doctor")}
          </div>
          <div>
    ...
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          ...
          <p style={styles.subtitle}>{patients.length} patient{patients.length !== 1 ? "s" : ""} avec accès accordé</p>
        </header>
        <div style={styles.searchRow}>
          <div style={styles.searchWrap}>
            <span>🔍</span>
            <input style={styles.searchInput} placeholder="Rechercher un patient..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
        {loading ? (
          ...
          <div style={styles.emptyState}>
            ...
            <p style={styles.emptyText}>{search ? "Aucun patient trouvé" : "Aucun patient ne vous a encore accordé l'accès"}</p>
          </div>
        ) : (
          <div style={styles.grid}>
            {filtered.map((access) => {
              ...
              return (
                <div key={access.id} style={styles.card} onClick={() => p?.id && nav("doctor-patient", { patientId: p.id })}>
                  <div style={styles.cardHeader}>
                    <div style={styles.avatar}>
                      <span style={{ fontSize: 28 }}>{getAvatarEmoji(p?.gender, "patient")}</span>
                    </div>
                  ...
                  <div style={styles.cardBody}>
                    ...
                    <div style={styles.metaRow}>
                      ...
                      {p?.blood && <span style={{ ...styles.meta, ...styles.bloodType }}>🩸 {p.blood}</span>}
                    </div>
                  </div>
...
const styles = {
  page: { display: "flex", minHeight: "100vh", background: "#f0f4f8", fontFamily: "'Segoe UI', system-ui, sans-serif", flexDirection: "row" },
  ...
  sidebarMobile: { position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 400, transform: "translateX(-100%)", transition: "transform 0.25s ease", display: "flex" },
  ...
  doctorAvatar: { width: 36, height: 36, borderRadius: "50%", background: "#1e4d7b", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 },
  ...
  avatar: { width: 52, height: 52, borderRadius: "50%", background: "#f0f4f8", display: "flex", alignItems: "center", justifyContent: "center" },
